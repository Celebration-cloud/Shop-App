...
const TabsLayout = () => {
  ...
  useEffect(() => {
    navigation.addListener("s", fetchCartItems);
    return () => {
      navigation.removeListener("s", fetchCartItems);
    };
  }, [fetchCartItems]);

  useEffect(() => {
    navigation.addListener("s", fetchShoppingItems);

    return () => {
      navigation.removeListener("s", fetchShoppingItems);
    };
  }, [fetchShoppingItems]);
