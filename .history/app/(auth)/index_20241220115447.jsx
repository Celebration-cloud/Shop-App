...
const LoginScreen = () => {
 ...
 const handleLogin = async (data) => { try { const userCredential = await signInWithEmailAndPassword(auth, data.email, data.password); const user = userCredential.user; dispatch(loginAction(user, router)); // router.navigate({ // pathname: "(home)", // params: { session: `${user.email} logged in Successfully` }, // }); console.log("Data:", `${user.email} logged in Successfully`); } catch (error) { console.error("Error logging in:", error.message); }};
