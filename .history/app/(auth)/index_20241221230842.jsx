...
import { Animated, Button, Dimensions, Image, Pressable, StyleSheet, Text, TextInput, ToastAndroid, View } from 'react-native'
...
console.log(`User: ${user}, Auth: ${isAuth}, idToken: ${token} logged in`);
  ...
  useEffect(() => { if (isAuthenticated && token !== null) { router.replace({ pathname: "(home)", params: { session: `logged in Successfully` }, }); } }, [isAuthenticated, token]);
  
    
  const handleLogin = (data) => {
    dispatch(loginAction(data, router));
    console.log("Data:", `${data.email} logged in Successfully`);
  };
