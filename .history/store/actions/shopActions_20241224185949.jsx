...
export const addOrderItem = (userData, user, token) => async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(
        `${SHOP_API}/api/shopItem/user/addOrder?totalAmount=${userData.totalAmount}&idToken=${token}&user=${user}`,
        { totalAmount: userData.totalSum, idToken: token, user: user } // Correctly pass the data in the body
      );
