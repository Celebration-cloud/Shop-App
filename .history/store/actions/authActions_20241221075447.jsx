...
export const loginAction = (userData, router) => async (dispatch, getState) => {
    try {
      ...
      try {
        ...
        /* The line `dispatch({type: "TOKEN", payload: user.getIdToken()});` is dispatching an action
        with type "TOKEN" and passing the user's ID token as the payload. This action is likely used
        to store the user's ID token in the Redux store for later use within the application.
        Storing the ID token in the Redux store can be useful for making authenticated API requests
        or managing user authentication state throughout the application. */
