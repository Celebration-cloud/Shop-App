...
export const signUpAction =
  (userData, router) => async (dispatch, getState) => {
    try {
      ...
      dispatch({ type: "TOKEN", payload: idToken });
...
export const sessionAction = () => async (dispatch, getState) => {
    try {
        ...
        if(response.data.idToken === null ||response.data.idToken === undefined){
          throw new Error("error session")
        }
        dispatch({type: "USER", payload: response.data.user})
        dispatch({type: "TOKEN", payload: response.data.idToken})
        dispatch({type: "AUTH", payload: true})
        dispatch({ type: "ERROR", payload: null });
        dispatch({ type: "LOADING", payload: false });
    } catch (error) {
...
export const logoutAction = (userData) => async (dispatch, getState) => {
    try {
        dispatch({ type: "LOADING", payload: true})
        // const response = await axios.post(`${SHOP_API}/api/auth/logout`);
        ...
        dispatch({ type: "LOADING", payload: false });
         ...
         dispatch({
           type: "ERROR",
           payload: `Error: Unable to logout, ${error.response.data.message}`,
         });
          ToastAndroid.show(
            `Error: Unable to logout, ${error.response.data.message}`,
            ToastAndroid.TOP,
            ToastAndroid.LONG
          );
         dispatch({ type: "LOADING", payload: false });
