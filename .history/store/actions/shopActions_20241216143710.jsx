...
export const getShopItems = () => async (dispatch, getState) => {
    try {
        ...
        const response = await axios.get(`${SHOP_API}/api/shopItem/user/gt`);
      ...
      console.log(message)
         dispatch({ type: "ERROR", payload: error.response.data.message.array() });
         dispatch({ type: "LOADING", payload: false });
    }finally{
        dispatch({ type: "LOADING", payload: false });
    }
...
export const addShopItem = (userData) => async (dispatch, getState) => {
  try {
    dispatch({ type: "LOADING", payload: true });
    const response = await axios.post(`${SHOP_API}/api/shopItem/post`); 
    dispatch({ type: "ERROR", payload: null });
    dispatch({ type: "LOADING", payload: false });
  } catch (error) {
