...
export const removeCartItem = (user, user, token) => async (dispatch, getState) => {
  console.log(`${user} deleted`);
  try {
    dispatch({ type: "LOADING", payload: true });
    const response = await axios.delete(
      `${SHOP_API}/api/shopItem/user/removeCart/${user}?idToken=${token}&user=${user}`
    );
    if (response.status === 400) {
      throw new Error(response.data.message);
    }
    //1efb9573-4a00-63d0-bf71-300280cd1963
    ToastAndroid.show(
