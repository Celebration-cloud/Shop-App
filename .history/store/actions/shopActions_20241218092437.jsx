...
export const editShopItem = (id, editData) => async (dispatch, getState) => {
  try {
    dispatch({ type: "LOADING", payload: true });
    const response = await axios.patch(`${SHOP_API}/api/shopItem/pat/${id}`, editData)
  ...
  const message =
    error.response.data[0]?.msg || error.response.data[0]?.msg || error.message;
    console.log("Update data error:", error.response?.data || error.message);
    console.log(message);
    dispatch({ type: "ERROR", payload: `${message}`});
    dispatch({ type: "LOADING", payload: false });
  } finally {
