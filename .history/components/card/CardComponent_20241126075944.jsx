...
const List = ({data}) => {
  ...
  const addToExitingData = cart.find((item) => item.id.includes(data.id))
  const handleDetails = () => { `details: ${console.log(data) }`};
  const handleCarts = () => {
    if(cart.length < 1){
      setCart(curr => {return [{...curr, count: 1data}]})
      console.log(cart)
      // dispatch({type: "CART", payload: [...curr, data]})
    }else if(addToExitingData){
      const exitingData = cart.map((item) => item.id === data.id ? {count: })
    }
  };
  return (
