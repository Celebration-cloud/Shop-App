...
const List = ({data}) => {
  ...
  const addToExitingData = cart.length > 1 ? cart.find((item) => item?.id.includes(data.id)) : []
  const handleDetails = () => { `details: ${console.log(data) }`};
  const handleCarts = () => {
    if(cart.length < 1 || !addToExitingData){
      setCart(cart => {return [{...cart, count: 1, ...data}]})
      // dispatch({type: "CART", payload: [...curr, data]})
    }else if(addToExitingData){
      const exitingData = cart.map((item) => item.id === data.id ? {...item, count: item.count++} : item)
      console.log(exitingData)
    }
    console.log("total ca"cart)
  };
  return (
